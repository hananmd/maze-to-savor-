...
// For loop detection
int visited_positions[20][3]; // Track up to 20 teleport positions
int visit_count = 0;
...
// Check for multiple stairs at this position
int found_stairs[10];
int stair_count = find_all_stairs_at(stairs, num_stairs, old_f, new_w, new_l, found_stairs);
...
int selected_stair;
if (stair_count == 1) {
selected_stair = found_stairs[0];
} else {
// Multiple stairs - select best one based on distance to flag
selected_stair = select_best_stair(stairs, found_stairs, stair_count, old_f, new_w, new_l, flag);
printf("Multiple stairs found, selecting best one based on distance to flag.\n");
}

Stair stair = stairs[selected_stair];
...
// Check for loop before teleporting
if (detect_teleport_loop(player, visited_positions, &visit_count, 20)) {
printf("Teleport loop detected! Moving player to starting area.\n");
handle_starting_area_logic(player, player_id);
return;
}
...
// Check if player ended up in starting area
if (player->pos[0] == 0 && 
player->pos[1] >= START_AREA_W_MIN && player->pos[1] <= START_AREA_W_MAX &&
player->pos[2] >= START_AREA_L_MIN && player->pos[2] <= START_AREA_L_MAX) {
player->in_game = 0;
printf("Player moved to starting area but retains movement points.\n");
}
}
continue;
}
